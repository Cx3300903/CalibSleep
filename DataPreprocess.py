...
if __name__ == "__main__":
    # ---------------------- 配置参数 ----------------------
    ROOT_DIR = "/data/home/chenxiang/osahs/Datasets/SleepStage_test"  # 所有受试者文件都在这里（无train/test子目录）
    SAVE_ROOT = "/data/home/chenxiang/osahs/Datasets/SleepStage_testSaved_NPY/Sleep_Test"      # 划分后的数据保存路径
    BATCH_SIZE = 32                                                 # 批次大小
    TRAIN_RATIO = 0.8                                               # 训练集比例（80%训练，20%测试）
    RANDOM_SEED = 2025                                              # 随机种子（保证划分可复现）
    
    # ---------------------- 步骤1：初始化生成器 ----------------------
    generator = MultiStudyDatasetGenerator(ROOT_DIR, CONFIG)
    
    # ---------------------- 步骤2：自动划分训练集/测试集 ----------------------
    # save_npy=True：保存划分后的数据集到SAVE_ROOT
    train_dataset, test_dataset = generator.generate_split_dataset(
    ...
    # ---------------------- 步骤4：验证数据加载器 ----------------------
    print(f"\n{'='*60}")
    print(f"数据加载器验证（仅EEG C3-A2和EOG LOC-A2，标签来自UserStaging）")
    print(f"{'='*60}")
    print(f"训练集总片段数: {len(train_dataset)}")
    print(f"测试集总片段数: {len(test_dataset)}")
    print(f"训练集批次数量: {len(train_loader)}")
    print(f"测试集批次数量: {len(test_loader)}")
    
    # 查看单个批次的数据格式
    for batch in train_loader:
        time_seq, freq_seq, labels, modal_mask, seg_idx = batch
        print(f"\n单个批次数据格式:")
        print(f"  时域信号形状: {time_seq.shape} → (批次大小, 上下文长度, 模态数(EEG/EOG), 信号长度)")
        print(f"  时频图像形状: {freq_seq.shape} → (批次大小, 上下文长度, 模态数, 频率点, 时间窗)")
        print(f"  标签形状: {labels.shape} → (批次大小,)")
        print(f"  模态掩码形状: {modal_mask.shape} → (批次大小, 模态数)")
        print(f"  标签示例: {labels[:10].numpy()}")
        print(f"  模态掩码示例: {modal_mask[:10].numpy()}")
        break
    
    print(f"\n✅ 数据集划分完成！训练集/测试集已保存至: {SAVE_ROOT}")
    print(f"✅ 数据加载器创建成功，可直接用于模型训练！")